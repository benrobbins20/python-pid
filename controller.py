...
class Controller:
    ...
    def __init__(self, starting_ref):
        ...
        self.hold_ratio = np.clip(self.gravity_hold / self.max_output, -0.999, 0.999) # strictly feed arctan < +-1
        ...
        # provides initial error state and sign, used for integral reset
        self.previous_error = options['SET_POINT'] - options['START_LOC']
    ...
    def run(self, t, x, v, dt=None):
        ...
        if dt is not None:
            ...
            # integral reset if sign changes (overshoot/undershoot)
            # regular state: +pos_error * +prev_error = + no sign change
            # up->down -pos_error * +prev_error = - sign change, below the target
            # down->up + +pos_error * -prev_error = - sign change, above the target
            if position_error * self.previous_error < 0:
                self.integral = 0
                
            # only accumulate steady state error < 1m
            ...
            # update previous error inside accepted timestep condition
